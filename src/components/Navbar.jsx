function Navbar() {
  return (
    <div className="fixed z-[999] w-full px-4 md:px-8 lg:px-20 py-4 md:py-6 lg:py-8 font-['Neue_Montreal'] flex justify-between items-center backdrop-blur-md">
      {/* Logo */}
      <div className="logo">
        <svg
          width="72"
          ...
          className="w-12 h-5 md:w-16 md:h-6 lg:w-[72px] lg:h-[30px]"
        >
          <path
      ...
      {/* Links */}
      <div className="links hidden sm:flex gap-2 md:gap-4 lg:gap-10">
        {["Services", "Our Work", "About Us", "Insights", "Contact"].map((item, index) => (
          <a
            key={index}
            href="#"
            className={`relative text-[10px] md:text-xs lg:text-sm xl:text-lg font-light capitalize group overflow-hidden ${
              index === 4 ? "ml-0 md:ml-12 lg:ml-28" : ""
            }`}
          >
            {/* Default Text (visible normally, moves up on hover) */}
            <span className="block transition-all duration-500 ease-[cubic-bezier(0.25,1,0.5,1)] group-hover:-translate-y-[120%] group-hover:opacity-0">
              {item}
            </span>

            {/* Hover Text (falls from top) */}
            <span className="absolute left-0 top-0 block translate-y-[-120%] opacity-0 transition-all duration-500 ease-[cubic-bezier(0.25,1,0.5,1)] group-hover:translate-y-0 group-hover:opacity-100">
              {item}
            </span>
